...
TEST_F(DownloadHistoryCacheTest, BusyWriteReturnsStructuredBusyError){ASSERT_TRUE(cache().Initialize(Path()).IsOk());sqlite3*db=nullptr;ASSERT_EQ(sqlite3_open(Path().c_str(),&db),SQLITE_OK);ASSERT_EQ(sqlite3_exec(db,"BEGIN IMMEDIATE",nullptr,nullptr,nullptr),SQLITE_OK);auto start=std::chrono::steady_clock::now();auto r=cache().AddRecord(Record());auto elapsed=std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now()-start);EXPECT_TRUE(r.HasError());EXPECT_EQ(r.GetError().primary_code,SQLITE_BUSY);EXPECT_GE(elapsed.count(),2700);EXPECT_LT(elapsed.count(),6000);sqlite3_exec(db,"ROLLBACK",nullptr,nullptr,nullptr);sqlite3_close(db);}
