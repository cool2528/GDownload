...
- **Fix macOS in-app updates failing with "An error occurred while launching the installer" after the download completed.** Two stacked faults: Sparkle's installer progress tool (`Updater.app`) was never shipped inside the app — a `*.app` ignore rule had silently kept it out of the repository since the day auto-update was added — so launching the installer always failed; and the DMG packaging step dereferenced every framework symlink while copying the app, which broke the app's code signature, so even a launched installer would have rejected the update as improperly signed. The missing tool is now bundled, the DMG copy preserves symlinks (`ditto`), and packaging now hard-fails if the final app's signature does not verify. **Existing macOS installs cannot self-heal — this one update must be installed manually from the downloaded DMG; automatic updates work again from the next release on.** / **修复 macOS 应用内更新下载完成后报 "An error occurred while launching the installer"。** 两个叠加故障：Sparkle 的安装进度组件(`Updater.app`)从未被打进应用 —— 自动更新功能引入当天就被 `*.app` 忽略规则静默挡在版本库之外，启动安装器因此必然失败；DMG 打包拷贝应用时把 framework 符号链接全部解引用，破坏了应用签名，即使安装器能启动也会被判"签名无效"拒绝安装。现已补齐缺失组件，DMG 拷贝改用保留符号链接的 `ditto`，且最终应用签名校验不通过时打包直接失败。**存量 macOS 安装无法自愈 —— 本次更新需手动下载 DMG 安装一次，此后自动更新恢复正常。**
