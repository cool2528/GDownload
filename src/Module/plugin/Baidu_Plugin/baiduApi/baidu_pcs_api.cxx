...
namespace gdl {
	namespace plugin {
		...
		bool BaiduPcsApi::VerifySharePassword(const std::string& surl, const std::string& pwd,
			...
			cpr::Parameters verify_parameters = {{"t", timestamp},	  {"surl", surl},
												 {"web", "1"},		  {"app_id", "250528"},
												 {"bdstoken", bds_token_},	  {"logid", log_id_string_},
		...
		std::optional<cpr::Response> BaiduPcsApi::FetchDownloadRequestSignature(const std::string& surl,
			...
			cpr::Parameters sig_request_parameters = {
				{"surl", surl},		 {"fields", "sign,timestamp"},
				{"view_mode", "1"},	 {"channel", "chunlei"},
				{"web", "1"},		 {"app_id", "250528"},
				{"bdstoken", bds_token_},	 {"logid", log_id_string_},
				{"clienttype", "0"}, {"dp-logid", detail::DpLogId().GetDpLogId()}};
			auto reply =
