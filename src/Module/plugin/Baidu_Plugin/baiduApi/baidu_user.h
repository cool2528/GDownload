...
namespace gdl {
    namespace plugin {
        static const std::vector<std::string> PHONE_MODEL_DATABASE = {
            "1501_M02",			// 360 F4
            "1503-M02",			// 360 N4
            "1505-A01",			// 360 N4S
      ...
      inline  static std::string calu_md5(const std::string& str) {
            unsigned char md5_hash[MD5_DIGEST_LENGTH];
            MD5(reinterpret_cast<const unsigned char*>(str.c_str()), str.length(), md5_hash);
        ...
        inline static std::string calu_sha1(const std::string& str) {
            unsigned char sha1_hash[SHA_DIGEST_LENGTH];
            SHA1(reinterpret_cast<const unsigned char*>(str.c_str()), str.length(), sha1_hash);

            std::stringstream ss;
            for (int i = 0; i < SHA_DIGEST_LENGTH; i++) {
                ss << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(sha1_hash[i]);
            }
            return ss.str();
        }

       inline static int64_t now_timestamp() {
       ...
       inline static std::string sum_IMEI(const std::string& key) {
        ...
        inline static std::string get_phone_model(const std::string& key) {
            if (PHONE_MODEL_DATABASE.empty()) {
                return "S3";
            }
		...
		inline static std::string quote_plus(const std::string& value) {
        ...
        class BaiduPCS {
           ...
           public:
			BaiduPCS() : user_id_(0), user_info_(nlohmann::json::object()) {}
            bool InitUserInfo(const std::string& bduss_str) {
                std::string bduss		 = bduss_str;
                std::string timestamp	 = std::to_string(now_timestamp());
                std::string model		 = get_phone_model(bduss);
                std::string phoneIMEIStr = sum_IMEI(bduss);
                std::map<std::string, std::string> data;
                data["bdusstoken"]		= bduss + "|null";
                data["channel_id"]		= "";
                data["channel_uid"]		= "";
                data["stErrorNums"]		= "0";
                data["subapp_type"]		= "mini";
                data["timestamp"]		= timestamp + "922";
                data["_client_type"]	= "2";
                ...
                std::string cuid_base =
                    bduss + "_" + data["_client_version"] + "_" + data["_phone_imei"] + "_" + data["from"];
                std::string cuid_md5 = calu_md5(cuid_base);
                std::transform(cuid_md5.begin(), cuid_md5.end(), cuid_md5.begin(), ::toupper);
                data["cuid"] = cuid_md5 + "|" + std::string(phoneIMEIStr.rbegin(), phoneIMEIStr.rend());
                ...
                std::string sign_base;
                for (const auto& key : keys) {
                    sign_base += key + "=" + data[key];
                }
                sign_base += "tiebaclient!!!";
                std::string sign = calu_md5(sign_base);
                ...
                if (resp.status_code == 200) {
                    try {
                        user_info_ =  json::parse(resp.text);
						if (user_info_.contains("user") && user_info_["user"].is_object()) {
							auto user = user_info_["user"];
							if (user.contains("id") && user["id"].is_string()) {
								user_id_ = std::stoull(user["id"].get<std::string>());
							}
							else if (user.contains("id") && user["id"].is_number()) {
								user_id_ = user["id"].get<std::uint64_t>();
							}
							else {
								return false;
							}
            ...
            json GetUserInfo() const { return user_info_; }
			std::uint64_t GetUserID() const { return user_id_; }
            bool IsValidate() const { return user_id_ > 0 && !user_info_.empty(); }
